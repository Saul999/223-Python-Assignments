...
class Flights:
    ...
    def add_flight(self, origin, destination, number, depature, next_day, arrival):
        if len(depature) != 4 or depature.isnumeric() is False:
            return False
        elif len(arrival) != 4 or arrival.isnumeric() is False:
            return False
        else:
            self.data.append([origin, destination, number, depature, next_day, arrival])
            self.write_data()
            return True
